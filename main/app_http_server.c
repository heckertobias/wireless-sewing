...
/* --- Bernina "media changed" notification ---------------------------------
 * A USB-MSC host (e.g. an embroidery machine) enumerates the FAT once and
 * never notices files that the web interface added or removed on the SD
 * card afterwards. A short VBUS-invalid pulse makes the native USB-OTG PHY
 * perform a soft disconnect/reconnect, so the host re-enumerates the MSC
 * device and re-reads the FAT - exactly as if the stick had been unplugged
 * and plugged back in, but without touching the cable. */
static void msc_notify_media_change(void)
{
    ESP_LOGI(TAG, "Notifying USB host of media change (soft USB reconnect)");
    usbd_vbus_enable(false);
    vTaskDelay(pdMS_TO_TICKS(CONFIG_MSC_MEDIA_CHANGE_DELAY_MS));
    usbd_vbus_enable(true);
}
#else
static void msc_notify_media_change(void)
{
    ESP_LOGW(TAG, "msc_notify_media_change() not supported on this target");
}
#endif
...
static esp_err_t upload_post_handler(httpd_req_t *req)
{
    ...
    if (bits & FILE_WRITE_SUCCESS_BIT) {
        ...
        /* Let the embroidery machine see the new file without unplugging */
        msc_notify_media_change();
...
static esp_err_t delete_post_handler(httpd_req_t *req)
{
    ...
    /* Let the embroidery machine see the file is gone without unplugging */
    msc_notify_media_change();
