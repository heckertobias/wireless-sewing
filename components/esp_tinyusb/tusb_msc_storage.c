...
esp_err_t tinyusb_msc_storage_mount(const char *base_path)
{
    ...
    FATFS *fs = NULL;
    /* ESP-IDF >= 6.0 changed esp_vfs_fat_register() to take a single
     * esp_vfs_fat_conf_t struct instead of separate base_path/drv/max_files
     * arguments (local patch for IDF 6.0.1, see CLAUDE.md "Vendored
